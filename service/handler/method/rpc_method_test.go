...
var (
	stringFn = func(ctx context.Context, req string, resp *string) error {
		*resp = "this is a string"
		return nil
	}
)
...
func Test_extractSig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := extractSig(tt.args.v, tt.args.d); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("extractSig() = %v, want %v", got, tt.want)
			}
		})
	}
}
